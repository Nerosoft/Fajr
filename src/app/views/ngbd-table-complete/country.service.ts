...
function matches(itemRow, term: string, pipe: PipeTransform) {
  if (Number.isInteger(itemRow[0][2]))
    return itemRow[0][1].toLowerCase().includes(term.toLowerCase())
      || pipe.transform(itemRow[0][0]).includes(term)
      || pipe.transform(itemRow[0][2]).includes(term)
  else
    return itemRow[0][1].toLowerCase().includes(term.toLowerCase())
      || pipe.transform(itemRow[0][0]).includes(term)
      || (itemRow[0][2] + "").toLowerCase().includes(term.toLowerCase());
}
